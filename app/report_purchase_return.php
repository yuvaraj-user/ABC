...
<?php 
    ...
    <strong>Success!</strong> Purchase Return are Added Successfully.
  </div>
	...
	{ ?>
		<div class="alert alert_msg alert-danger alert-dismissable">
    <a href="#" class="close" data-dismiss="alert" aria-label="close">×</a>
    <strong>Success!</strong> Purchase Return are Removed Successfully.
	</div>
	<?php } 
	?>
	<div class="panel-heading lead ">
		<div class="row">
			<div class="col-lg-8 col-md-8">Purchase Return Report</div>				
...
<?php
								...
								if(isset($_REQUEST['filter']))
								{
								 ...
								 $product=$_REQUEST['product'];
											...
											if(empty($product)&& empty($date)&& empty($supplier))
				
											{
									             $query_getall2="select p.*,s.Name as supplier_name,t.Name as product_Name from tbl_purchase_return p left join tbl_supplier s on p.Supplier_Id=s.Id left join tbl_product t on t.Id=p.Product_Id where p.Status='Active'";
											}
											else
											{
												 $query_getall2.="select p.*,s.Name as supplier_name,t.Name as product_Name from tbl_purchase_return p left join tbl_supplier s on p.Supplier_Id=s.Id left join tbl_product t on t.Id=p.Product_Id where p.Status='Active' AND  ";
												$counter=0;
											} 											
											if(!empty($product))
											{
											...
											if(!empty($supplier))
											{
												$qarr[$counter]="p.Supplier_Id='$supplier'";
												$counter++;
											}
						...
						while($fetch=mysqli_fetch_array($qry_receipt))
							{
								...
								$Purchase_Invoice_No = $fetch['Purchase_Invoice_No'];
							?>
							<tr>
								<td><?php echo $i;?></td>
								...
								<td><button type="button"  class="btn btn-danger" onclick="window.location.href='delete_purchase_return.php?do=delete&id=<?php echo $id; ?>'"><a style=" text-decoration: none; color:#FFF" >Inactive&nbsp;</a></button>
		</td>
							</tr>
