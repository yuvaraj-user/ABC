...
<?php 
    ...
    <strong>Success!</strong> Sales Return are Added Successfully.
  </div>
	...
	{ ?>
		<div class="alert alert_msg alert-danger alert-dismissable">
    <a href="#" class="close" data-dismiss="alert" aria-label="close">×</a>
    <strong>Success!</strong> Sales Return are Removed Successfully.
	</div>
	<?php } 
	?>
	<div class="panel-heading lead ">
		<div class="row">
			<div class="col-lg-8 col-md-8">Purchase Sales Report</div>				
...
<?php
								...
								if(isset($_REQUEST['filter']))
								{
								 ...
								 $product=$_REQUEST['product'];
											...
											if(empty($product)&& empty($date)&& empty($customer))
				
											{
									             $query_getall2="select p.*,s.Name as supplier_name,t.Name as product_Name,k.Name as shop_name from tbl_sales_return p left join tbl_coordinator s on p.Customer_Id=s.Id left join tbl_shop k on k.Id=p.Shop_Id left join tbl_product t on t.Id=p.Product_Id where p.Status='Active'";
											}
											else
											{
												 $query_getall2.="select p.*,s.Name as supplier_name,t.Name as product_Name,k.Name as shop_name from tbl_sales_return p left join tbl_coordinator s on p.Customer_Id=s.Id left join tbl_shop k on k.Id=p.Shop_Id left join tbl_product t on t.Id=p.Product_Id where p.Status='Active' AND  ";
												$counter=0;
											} 											
											if(!empty($product))
											{
											...
											if(!empty($customer))
											{
												$qarr[$counter]="p.Customer_Id='$customer'";
												$counter++;
											}
						...
						while($fetch=mysqli_fetch_array($qry_receipt))
							{
								...
								$Sales_Invoice_No = $fetch['Sales_Invoice_No'];
