...
<div class="row panel panel-primary"> 
	<div class="panel-heading lead ">
		<div class="row">
			<div class="col-lg-8 col-md-8">Indent Report</div>				
...
<?php
								...
								if(isset($_REQUEST['filter']))
								{
								 ...
								 $to = date("d-m-Y", strtotime($dto));
											...
											{
									             $query_getall2="SELECT p.*,s.Name as sup_name,t.Name as pro_name,s.Name as shop_name FROM tbl_sales_order p LEFT JOIN tbl_coordinator s on s.Id=p.Customer_Id LEFT JOIN tbl_shop k on k.Id=p.Shop_Id LEFT JOIN tbl_product t on t.Id=p.Product_Id WHERE p.Status='Active' AND STR_TO_DATE(p.`Date`,'%d-%m-%Y') BETWEEN STR_TO_DATE('$from', '%d-%m-%Y') AND STR_TO_DATE('$to', '%d-%m-%Y')";
											}
											else
											{
												  $query_getall2="SELECT p.*,s.Name as sup_name,t.Name as pro_name,s.Name as shop_name FROM tbl_sales_order p LEFT JOIN tbl_coordinator s on s.Id=p.Customer_Id LEFT JOIN tbl_shop k on k.Id=p.Shop_Id LEFT JOIN tbl_product t on t.Id=p.Product_Id WHERE p.Status='Active' AND p.Customer_Id='$product' AND STR_TO_DATE(p.`Date`,'%d-%m-%Y') BETWEEN STR_TO_DATE('$from', '%d-%m-%Y') AND STR_TO_DATE('$to', '%d-%m-%Y')";
											} 											
						...
						while($fetch=mysqli_fetch_array($qry_receipt))
							{
								...
								$qury_opn="select SUM(Quantity) as qty from tbl_sales_order where Product_Id='$product_id' AND Status='Active' AND Id='$order_id'";
								$qury_exe_opn=mysqli_query($con,$qury_opn);
								$fetch_opn=mysqli_fetch_array($qury_exe_opn);
								$opening_qty = $fetch_opn['qty'];
								
								
								$qury="select SUM(Quantity) as qty from tbl_sales where Product_Id='$product_id' AND Status='Active' AND Sales_Order_No='$order_no'";
