...
<div class="row panel panel-primary"> 
	<div class="panel-heading lead ">
		<div class="row">
			<div class="col-lg-8 col-md-8">Customer Outstanding Report</div>				
...
<?php
								...
								if(isset($_REQUEST['filter']))
								{
								 $customer=$_REQUEST['customer'];
											...
											{
									             $query_getall2="select p.*,SUM(p.Net_Amount) as bill_value,s.Name as customer from tbl_sales p left join tbl_customer s on p.Customer_Id=s.Id where p.Status='Active' GROUP BY p.Invoice_No";
											}
											else
											{
												 $query_getall2="select p.*,SUM(p.Net_Amount) as bill_value,s.Name as customer from tbl_sales p  left join tbl_customer s on p.Customer_Id=s.Id where p.Status='Active' AND p.Customer_Id='$customer' GROUP BY p.Invoice_No";
						...
						while($fetch=mysqli_fetch_array($qry_receipt))
							{
								...
								$select_GrpQry=mysqli_query($con,"SELECT SUM(Purchase_Wise_Amount) as paid_amount FROM `tbl_receipts` WHERE `Invoice_No_Purchase` = '$Invoice_No'");
