...
if(isset($_REQUEST['submit']))
{
	...
	$query_aadh="SELECT COUNT('Id') as cid FROM tbl_group WHERE Name ='$name' AND Status='Active'";
$query_aadh_res = mysqli_query($con, $query_aadh); 
 $fetch_cnt = mysqli_fetch_array($query_aadh_res);
 
 if($fetch_cnt['cid'] ==0) {
		
		$insert_details = mysqli_query($con,"INSERT INTO `tbl_group`(`Name`, `Parent`, `Status`, `Created_On`, `Created_By`) VALUES ('$name','$parent','$status','$createdon','$createdby')");
		
		if($insert_details){
			 echo '<script type="text/javascript">
					window.location.replace("view_group.php?step=suces");
					</script>';	
     ...
     echo '<script type="text/javascript">
					window.location.replace("view_group.php?step=duplicate");
					</script>';
}
}
?>
...
<div class="panel-heading lead ">
	<div class="row">
		<div class="col-lg-8 col-md-8"><b>Add Sector</b></div>				
