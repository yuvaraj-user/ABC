...
<?php
                ...
                ?>
                    <div class="alert alert_msg alert-success alert-dismissable">
                        ...
                        <strong>Success!</strong> work order was Added Successfully.
                    </div>

                <?php
                } else if ($final == "decline") {
                ?>
                    <div class="alert alert_msg alert-success alert-dismissable">
                        <a href="#" class="close" data-dismiss="alert" aria-label="close">×</a>
                        <strong>Success!</strong> work order was decline Successfully.
                    </div>
                ...
                <?php } else if ($final == "delete") { ?>
                    <div class="alert alert_msg alert-danger alert-dismissable">
                        <a href="#" class="close" data-dismiss="alert" aria-label="close">×</a>
                        <strong>Success!</strong> work order are Removed Successfully.
                    </div>
	...
	<div class="panel-heading lead ">
		<div class="row">
			<div class="col-lg-8 col-md-8">Work Order Report</div>				
			...
			<label class="control-label col-sm-2">Date<font color="red">&nbsp;*&nbsp;</font></label>
				<div class="form-group col-sm-4">
					<input type="text" name="date" id="reportrange" class="form-control pull-right" placeholder="pick the date dd-mm-yyy" required>
                        ...
                        <div class="form-group col-sm-4">
                            <select class="form-control selectpicker" name="status" id="customer" data-live-search="true">
...
<?php
								...
								if(isset($_REQUEST['filter']))
								{
								 ...
								 $to = date("d-m-Y", strtotime($dto));							 
												 ...
												 $query_getall2.="SELECT wo.*,wo.Id as wid,wo.Status as sta,wo.Date as wo_dt,so.*,so.Date as so_dt,l.*,l.Id as ld_id,e.Name as emp_name,t.Name as cus_name,l.Date as ld_dt,p.*,i.*,c.*,t.* FROM tbl_work_order wo left join tbl_sales_order so on so.Id=wo.Sale_Order_Id left join  tbl_lead l on so.Lead_Id=l.Id LEFT JOIN tbl_lead_products p on l.Id=p.Lead_Id LEFT JOIN tbl_stock_item i on i.Id=p.Product_Id LEFT JOIN tbl_customer c on c.Id=l.Customer_Id LEFT JOIN tbl_contact_person t on t.Customer_Id=c.Id LEFT JOIN tbl_employee e on l.Followup_Employee=e.Id WHERE ";
												$counter=0;
											...
											if(!empty($status_type))
											{
												...
												$qarr[$counter]="wo.Status	='$status_type'";
												$counter++;
											}
...
<br>
			...
			$total_percenatge = ($paid/$tot_amt)*100
							...
							<tr>
								<td><?php echo $k;?></td>
								<td><?php echo $fetch['Work_Order_No'];?></td>
								...
								<td><?php echo $total_percenatge;?></td>
								<td><?php echo $paid;?></td>
	...
	<td>
	    ...
	    if($status=='Pending') {
	    ?>
	    <a href="approve_wo.php?id=<?php echo $wid; ?>" class="btn btn-info" target="_blank">Approve</a>
							<a href="decline_wo.php?id=<?php echo $wid; ?>" class="btn btn-warning" target="_blank">Decline</a>
							<?php } else if($status=='Active') { ?>
							<h4 style="color:green"><b>Approved</b> 
							</h4> <a href="decline_wo.php?id=<?php echo $wid; ?>" class="btn btn-warning" target="_blank">Decline</a>
							<?php } else if($status=='Decline') { ?>
								<h4 style="color:red"><b>Declined</b> </h4>
								 <a href="approve_wo.php?id=<?php echo $wid; ?>" class="btn btn-info" target="_blank">Re-Approval</a>
							<?php }  ?>
								</td>
    ...
    $('#example').DataTable( {
			...
			"scrollCollapse": true,
			  "lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
			   ...
			   {
					extend: 'copyHtml5',
					title: 'Work Order Report - <?php echo $bname;?>' 			
				},
				{
					extend: 'pdfHtml5',
					title: 'Work Order Report - <?php echo $bname;?>' 			
