...
$salary_data_get  = mysqli_query($con, "select * from tbl_salary where id = '$sid'");
$salaryGet = mysqli_fetch_array($salary_data_get);
...
if (isset($_REQUEST['update'])) {
    ...
    $alreadychk  = mysqli_query($con, "select * from tbl_salary where id = '$sid'");
    $salary_data = mysqli_fetch_array($alreadychk);

    $fetchcnt = mysqli_num_rows($alreadychk);
    if ($fetchcnt > 0) {
        $update_details = mysqli_query($con, "UPDATE `tbl_salary` SET `employee` = '$employee',`date` = '$date',`attendance` = '$attendance',`salary` = '$salary',`incentive` = '$incentive',`esi` = '$esi',`pf` = '$pf',`status` = 'Active' where id='$sid'");
        if ($update_details) {
            echo '<script type="text/javascript">
					window.location.replace("view_salary.php?step=update");
					</script>';
        }
    }
}
        ...
        <div class="content-wrapper">
            <div class="row panel panel-primary">
                <div class="panel-heading lead ">
                    <div class="row">
                        <div class="col-lg-8 col-md-8"><label>Edit Salary Details</label></div>
					...
					<?php /*
                        <div class="col-sm-12 form-group">
                            ...
                            <div class="col-sm-10">
                                ...
                                <select id="employee" name="employee" class="form-control selectpicker" data-live-search="true" onchange="getAuctionchit(this.value);" required>
                                    ...
                                    $query_Applic = "select Id,Name from tbl_employee where Status='Active'";
                                    ...
                                    ?>
                                        <option data-emp-id="<?php echo $fetch_appc_id; ?>" value="<?php echo $fetch_appc_id; ?>" <?php if ($fetch_appc_id == $data['id']) { ?> selected="selected" <?php } ?>><?php echo $fetch_appc_Name;  ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                        </div>
						...
						*/ ?>
						...
						<div class="col-sm-12 form-group">
                            ...
                            <div class="col-sm-10">
                                ...
                                <select id="employee" name="employee" class="form-control selectpicker" data-live-search="true" onchange="getAuctionchit(this.value);" required>
                                    ...
                                    ?>
                                        <option data-emp-id="<?php echo $fetch_appc_emp_id; ?>" value="<?php echo $fetch_appc_id; ?>"  <?php if ($fetch_appc_emp_id == $salaryGet['employee']) { ?> selected="selected" <?php } ?> ><?php echo $fetch_appc_Name;  ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                        </div>
						...
						<div class="col-sm-12 form-group">
                            ...
                            <div class="col-sm-10">
                                ...
                                <select id="attendance" name="attendance_type" class="form-control selectpicker" required>
                                    ...
                                    ?>
                                        <option class="attendance_opt" data-from="<?php echo $fetch_attend_from; ?>" data-to="<?php echo $fetch_attend_to; ?>" value="<?php echo $fetch_attend_id; ?>" <?php if ($fetch_attend_id == $salaryGet['attendence_type']) { ?> selected="selected" <?php } ?> ><?php echo $fetch_attend_Name;  ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-12 form-group">
                            <label class="col-sm-2 control-label">Date<font color="red">&nbsp;*&nbsp;</font></label>
                            <div class="col-sm-4">
                                <input class="form-control dp" placeholder="yyyy-mm-dd" id="dob_F" name="date" value="<?php echo $salaryGet['date']; ?>" required readonly>
                            </div>
                            <label class="col-sm-2 control-label">Attendance ( % )<font color="red">&nbsp;*&nbsp;</font></label>
                            <div class="col-sm-4">
                                <input type="text" class="form-control" placeholder="Attendance" name="attendance" value="<?php echo $salaryGet['attendance']; ?>" required>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-12 form-group">
                            ...
                            <div class="col-sm-4">
                                <input type="text" class="form-control" placeholder="Salary" name="salary" id="basic_pay" value="<?php echo $salaryGet['salary']; ?>" required>
                            </div>
                            <label class="col-sm-2 control-label">Incentive<font color="red">&nbsp;*&nbsp;</font></label>
                            <div class="col-sm-4">
                                <input type="text" id="incentive" class="form-control" placeholder="Incentive" name="incentive" value="<?php echo $salaryGet['incentive']; ?>" required>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-12 form-group">
                            ...
                            <div class="col-sm-4">
                                <input type="text" id="esi"  class="form-control" placeholder="ESI" name="esi" value="<?php echo $salaryGet['esi']; ?>" required>
                            </div>
                            <label class="col-sm-2 control-label">PF<font color="red">&nbsp;*&nbsp;</font></label>
                            <div class="col-sm-4">
                                <input type="text" id="pf" class="form-control" placeholder="PF" name="pf" value="<?php echo $salaryGet['pf']; ?>" required>
                            </div>
                        </div>
						 ...
						 <div class="col-sm-12 form-group">
                            ...
                            <div class="col-sm-4">
                                <input type="text" id="ot" class="form-control" placeholder="OT WAGES" name="ot_wages" value="<?php echo $salaryGet['ot_wages']?>" required>
                            </div>
						</div>
                        ...
                        <div class="col-sm-12">
                            <br>
                            <div class="col-sm-6">
                                <button type="submit" name="update" class="pull-right center-block btn btn-primary">Submit</button>
                            </div>
                            <div class="col-sm-6">
		...
		$('#attendance').on('change',function(){
			...
			$.ajax({
				...
				success:function(res) {
					...
					$('#basic_pay').val(parseFloat(res.salary).toFixed(2));
					$('#attendance_percent').val(parseFloat(res.attendance_percent).toFixed(2));
					$('#pf').val(parseFloat(res.PF).toFixed(2));
					$('#esi').val(parseFloat(res.ESI).toFixed(2));
					$('#ot').val(parseFloat(res.ot_wages).toFixed(2))
				}
			});
		});
    </script>
