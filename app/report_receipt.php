...
<?php 
    ...
    <strong>Success!</strong> Receipts are Added Successfully.
  </div>
	...
	{ ?>
		<div class="alert alert_msg alert-danger alert-dismissable">
    <a href="#" class="close" data-dismiss="alert" aria-label="close">×</a>
    <strong>Success!</strong> Receipts are Removed Successfully.
	</div>
	<?php } 
	?>
	<div class="panel-heading lead ">
		<div class="row">
			<div class="col-lg-8 col-md-8">Receipt Report</div>				
...
<?php
								...
								if(isset($_REQUEST['filter']))
								{
								 ...
								 $to = date("d-m-Y", strtotime($dto));	
											...
											{
									             $query_getall2="select p.*,s.Company_Name as Company_Name from tbl_receipts p left join tbl_customer s on p.Customer_Id=s.Id where p.Status='Active'";
											}
											else
											{
												 $query_getall2.="select p.*,s.Company_Name as Company_Name from tbl_receipts p left join tbl_customer s on p.Customer_Id=s.Id where p.Status='Active' AND  ";
												$counter=0;
											} 			
											if(!empty($date))
											{
											...
											if(!empty($supplier))
											{
												$qarr[$counter]="p.Customer_Name='$customer'";
												$counter++;
											}
						...
						while($fetch=mysqli_fetch_array($qry_receipt))
							{
								$id = $fetch['Id'];
								$Date = $fetch['Date'];
								$supplier_name = $fetch['Company_Name'];
								$Invoice_No = $fetch['Invoice_No_Purchase'];
								$Mode_Of_Payment = $fetch['Mode_Of_Payment'];
								$Payment_Amount = $fetch['Total_Amount'];
								$Cheque_No = $fetch['Cheque_No'];
								$Cheque_Date = $fetch['Cheque_Date'];
								$Cheque_Bank_Name = $fetch['Cheque_Bank_Name'];
								$Card_No = $fetch['Card_No'];
								$Card_Bank_Name = $fetch['Card_Bank_Name'];
								$Paytm_Amount = $fetch['Paytm_Amount'];
								$Googlepay_Amount = $fetch['Googlepay_Amount'];
								$Reference_No = $fetch['Reference_No'];
								
								$Cash_Amount = $fetch['Cash_Amount'];
							    $Card_Amount = $fetch['Card_Amount'];
							    $Cheque_Amount = $fetch['Cheque_Amount'];
							   ...
							   if($Cash_Amount!='0.00'){
							       ...
							       $mode = 'Bank';
							       if($Cheque_Amount!='0.00'){
							           $nature = "cheque";
							       } else if($Card_Amount!='0.00'){
							            $nature = "neft";
							       } else if($Paytm_Amount!='0.00'){
							            $nature = "paytm";
 							       } else if($Googlepay_Amount!='0.00'){
 							           $nature = "googlepay";
 							       }
							   }
							...
							<tr>
								...
								<td><button type="button"  class="btn btn-info" onclick="window.location.href='receipt_print_sales.php?inv_no=<?php echo $Invoice_No; ?>'"><a style=" text-decoration: none; color:#FFF" >Print</a></button>
