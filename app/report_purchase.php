...
<?php 
	...
	<div class="panel-heading lead ">
		<div class="row">
			<div class="col-lg-8 col-md-8">Purchase Report</div>				
...
<?php
								...
								if(isset($_REQUEST['filter']))
								{
								 ...
								 $product=$_REQUEST['product'];
											...
											{
									             $query_getall2="select p.*,p.Id as pid,s.Name as supplier_name,t.Name as product_Name from tbl_purchase p left join tbl_supplier s on p.Supplier_Name=s.Id left join tbl_product t on t.Id=p.Product_Id where p.Status='Active'";
											}
											else
											{
												 $query_getall2.="select p.*,p.Id as pid,s.Name as supplier_name,t.Name as product_Name from tbl_purchase p left join tbl_supplier s on p.Supplier_Name=s.Id left join tbl_product t on t.Id=p.Product_Id where p.Status='Active' AND  ";
												$counter=0;
											} 											
											if(!empty($product))
											{
						...
						while($fetch=mysqli_fetch_array($qry_receipt))
							{
								...
								$Supplier_Date = $fetch['Supplier_Date'];
								$Supplier_Invoice_No = $fetch['Supplier_Invoice_No'];
