...
<div class="row panel panel-primary"> 
	<div class="panel-heading lead ">
		<div class="row">
			<div class="col-lg-8 col-md-8"> Shop Indent vs Sales Report</div>				
...
<?php
								...
								if(isset($_REQUEST['filter']))
								{
								 ...
								 $to = date("d-m-Y", strtotime($dto));							 
											...
											{
									             $query_getall2="SELECT p.*,p.Order_No as oid,s.Id as sid,p.Quantity as qty,s.Name as sup_name,t.Name as pro_name,g.Name as grp_name,d.Name as dist_Name,f.Name as gn_name FROM tbl_sales_order p LEFT JOIN tbl_shop s on s.Id=p.Shop_Id LEFT JOIN tbl_coordinator f on f.Id=s.Coordinator_Id LEFT JOIN tbl_product t on t.Id=p.Product_Id LEFT JOIN tbl_group g on g.Id=s.Group_Id LEFT JOIN tbl_district d on d.Id=s.District_Id WHERE p.Status='Active' AND STR_TO_DATE(p.`Date`,'%d-%m-%Y') BETWEEN STR_TO_DATE('$from', '%d-%m-%Y') AND STR_TO_DATE('$to', '%d-%m-%Y')";
											}
											else
											{
												  $query_getall2="SELECT p.*,p.Order_No as oid,s.Id as sid,p.Quantity as qty,s.Name as sup_name,t.Name as pro_name,g.Name as grp_name,d.Name as dist_Name,f.Name as gn_name FROM tbl_sales_target_shop p LEFT JOIN tbl_shop s on s.Id=p.Shop_Id LEFT JOIN tbl_coordinator f on f.Id=s.Coordinator_Id LEFT JOIN tbl_product t on t.Id=p.Product_Id LEFT JOIN tbl_group g on g.Id=s.Group_Id LEFT JOIN tbl_district d on d.Id=s.District_Id WHERE p.Status='Active' AND p.Customer_Id='$product' AND STR_TO_DATE(p.`Date`,'%d-%m-%Y') BETWEEN STR_TO_DATE('$from', '%d-%m-%Y') AND STR_TO_DATE('$to', '%d-%m-%Y')";
											} 											
						...
						while($fetch=mysqli_fetch_array($qry_receipt))
							{
								...
								$Customer_Id = $fetch['Shop_Id'];
								$grp_name = $fetch['grp_name'];
								$dist_Name = $fetch['dist_Name'];
								$gn_name = $fetch['gn_name'];
								$indent_qty = $fetch['qty'];
								$sid = $fetch['sid'];
								$j=1;
								$qury_opn_sale="select * from tbl_sales where Status='Active' AND Sales_Order_No='$order_no' AND Shop_Id='$sid'";
								$qury_exe_opn_sale=mysqli_query($con,$qury_opn_sale);
								while($fetch_opn_sale=mysqli_fetch_array($qury_exe_opn_sale))
								{
								$opening_qty_sale = $fetch_opn_sale['Quantity'];
								$opening_date_sale = $fetch_opn_sale['Date'];
								$opening_invoice_no = $fetch_opn_sale['Invoice_No'];
							?>
							<tr>
								<td><?php echo $j;?></td>
								...
								<td><?php echo $opening_date_sale;?></td>
								<td><?php echo $opening_invoice_no;?></td>
    ...
    $('#example1').DataTable( {
			...
			"scrollCollapse": true,
			  "lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
			   ...
			   {
					extend: 'copyHtml5',
					title: 'Shop Indent vs Sales Report' 			
				},
				{
					extend: 'excelHtml5',
					title: 'Shop Indent vs Sales Report' 			
