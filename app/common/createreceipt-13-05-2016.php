...
$emp_branch=$fetchemployee['Branch'];
	...
	function sendmessage($mobile,$appid,$doj,$cid,$amount,$cusname)
	{
		...
		$d1=strtotime("+1 month");
				...
				$select3=mysqli_query($con,"SELECT Scheme_Under_Category,Payment_Type,Date_Introducer,Is_Scheme FROM `tbl_chit_details` where Cust_Id='$cid'");
...
if($doj1==$dtest && $doj2==$dtest2 && $doj3==$dtest3)
{
	...
	$to1 =date("$doj1-m-Y",$d1);
				...
				$result2 = mysqli_query($con, $sql2) or die("Error1 in Selecting " . mysqli_error($con)); 
				...
				$check = "-";
		$companynamedel="From Sree Gokula Jayam Chit Funds Pvt Ltd.,";
   ...
   <REQUESTDATA>
	...
	if (empty($result)) 
	{
		$sql = "INSERT INTO `tbl_receipt_transaction`
				...
				$result = mysqli_query($con, $sql) or die("Error2 in Selecting " . mysqli_error($con));

		die(curl_error($ch));
		curl_close($ch); 
		if($sendsms =="Yes")
	{
		sendmessage($mobile,$appid,$doj,$cid,$amount,$cusname);	
	}
	} else 
	{
		...
		if (empty($info['http_code'])) 
		{
			$sql = "INSERT INTO `tbl_receipt_transaction`
				...
				VALUES ('$name1','$amount','$amount',
				'$date','No','$accountname','$date','0','0','Active','$empid','$emp_branch','$cid','$Payment_type')";
				$result = mysqli_query($con, $sql) or die("Error3 in Selecting " . mysqli_error($con));
			...
			if($sendsms =="Yes")
	{
		...
		} else 
		{
			...
			if($uniname1 == 1)
			{
				...
				$result2 = mysqli_query($con, $sql) or die("Error4 in Selecting " . mysqli_error($con));
	if($sendsms =="Yes")
	{
		sendmessage($mobile,$appid,$doj,$cid,$amount,$cusname);	
	}
			...
			}else
			{
				...
				VALUES ('$name1','$amount','$amount',
				'$date','No','$accountname','$date','0','0','Active','$empid','$emp_branch','$cid','$Payment_type')";
				$result2 = mysqli_query($con, $sql) or die("Error5 in Selecting " . mysqli_error($con));
