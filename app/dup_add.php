...
if(isset($_REQUEST['submit']))
{
    ...
    for($i=0;$i<=count($sales_id);$i++){
        ...
        if($netamount_sd !=0) {
			$sgst = $gst_amt_sd/2;
			$insert_details = mysqli_query($con,"INSERT INTO `tbl_call_register`(`Customer_Id`,`sales_id`,`Call_No`, `Date`, `Product_Id`, `Quantity`, `Rate`,  `Net_Amount`, `inv_date`, `inv_no`,`remark`,`Created_By`, `Status`, `Follow_By`,`count1`,`Cgst`,`Sgst`,`Igst`,`Amount`) VALUES ('$customer','$sales_id_sd','$Call_No','$enq_dates','$product_sd','$quantity_sd','$rate_sd','$netamount_sd','$inv_dat_sd','$inv_no_sd','$remark_sd','$createdby','$status','$follow_by','$i','$sgst','$sgst','$gst_amt_sd','')");
