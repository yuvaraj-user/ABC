...
<div class="row panel panel-primary"> 
	<div class="panel-heading lead ">
		<div class="row">
			<div class="col-lg-8 col-md-8"> Shopwise Target vs Achieved</div>				
						...
						while($fetch=mysqli_fetch_array($qry_receipt))
							{
								...
								$qury_opn="select SUM(Quantity) as qty from tbl_sales_target_shop where Product_Id='$product_id' AND Status='Active' AND Shop_Id='$Customer_Id' AND STR_TO_DATE(`Start_Date`,'%d-%m-%Y') BETWEEN STR_TO_DATE('$from', '%d-%m-%Y') AND STR_TO_DATE('$to', '%d-%m-%Y')";
								$qury_exe_opn=mysqli_query($con,$qury_opn);
								$fetch_opn=mysqli_fetch_array($qury_exe_opn);
								$opening_qty = $fetch_opn['qty'];
								
								$qury_opn_sale="select SUM(Quantity) as qty from tbl_sales where Product_Id='$product_id' AND Status='Active' AND Shop_Id='$Customer_Id' AND STR_TO_DATE(`Date`,'%d-%m-%Y') BETWEEN STR_TO_DATE('$from', '%d-%m-%Y') AND STR_TO_DATE('$to', '%d-%m-%Y')";
								...
								$qury_ind="select SUM(Quantity) as qty from tbl_sales_order where Product_Id='$product_id' AND Status='Active' AND Shop_Id='$Customer_Id' AND STR_TO_DATE(`Date`,'%d-%m-%Y') BETWEEN STR_TO_DATE('$from', '%d-%m-%Y') AND STR_TO_DATE('$to', '%d-%m-%Y')";
