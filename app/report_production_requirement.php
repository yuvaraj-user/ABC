...
<?php 
	...
	<div class="panel-heading lead ">
		<div class="row">
			<div class="col-lg-8 col-md-8">Production Requirement Report</div>				
			...
			<label class="control-label col-sm-2">Order Date<font color="red">&nbsp;*&nbsp;</font></label>
				<div class="form-group col-sm-4">
					<input type="text" name="date" id="reportrange" class="form-control pull-right" placeholder="pick the date dd-mm-yyy" required>
						...
						<label class="control-label col-sm-2">Machine Name </label>
...
<?php
								...
								if(isset($_REQUEST['filter']))
								{
								 ...
								 $product=$_REQUEST['product'];
											...
											{
									             $query_getall2="select p.*,s.Name as customer_name,t.Name as product_Name,k.Name as shop_name from tbl_sales p left join tbl_shop k on k.Id=p.Shop_Id left join tbl_sales_order y on y.Id=p.Sales_Order_No left join tbl_coordinator s on p.Customer_Id=s.Id left join tbl_product t on t.Id=p.Product_Id where p.Status='Active'";
											}
											else
											{
												 $query_getall2.="select p.*,s.Name as customer_name,t.Name as product_Name,k.Name as shop_name from tbl_sales p left join tbl_shop k on k.Id=p.Shop_Id left join tbl_sales_order y on y.Id=p.Sales_Order_No left join tbl_coordinator s on p.Customer_Id=s.Id left join tbl_product t on t.Id=p.Product_Id where p.Status='Active' AND  ";
												$counter=0;
											} 											
											if(!empty($product))
											{
					 ...
					 <table id="example1" class="table table-responsive table-bordered table-striped">
						...
						while($fetch=mysqli_fetch_array($qry_receipt))
							...
							<tr>
								...
								<td><?php echo $Date;?></td>
								<td><?php echo $fetch['Sales_Order_No']; ?></td>
