...
<?php 
	...
	<div class="panel-heading lead ">
		<div class="row">
			<div class="col-lg-8 col-md-8">Order Creation Report</div>				
	...
	<div class="panel-body"> 
	 <form class="form-horizontal" name="addaccount_details" action="" method="post" enctype="multipart/form-data" >
		<div class="col-lg-12 col-sm-12"> 
			<label class="control-label col-sm-2">Order Date<font color="red">&nbsp;*&nbsp;</font></label>
						...
						while($fetch=mysqli_fetch_array($qry_receipt))
							{
								...
								$mac_type = $fetch['machine_type'];
								$payment_type = $fetch['payment_type'];
								
							$branchlevel=mysqli_query($con,"select Name from tbl_employee where id='$employee'");
							$brnlevel=mysqli_fetch_array($branchlevel);
							$emp_name = $brnlevel['Name'];
							
							$state_level=mysqli_query($con,"select Name from tbl_product where id='$machine_id'");
							$state_level_fetch=mysqli_fetch_array($state_level);
							$mac_name = $state_level_fetch['Name'];    
							
							if($mac_type==1){
								$machine_type = "Standard";
							} else if($mac_type==2){
								$machine_type = "Standard Heavy";
							} else {
								$machine_type = "Customized";
							}
							
							if($payment_type==1){
								$payment_type = "Direct Pay";
							} else if($payment_type==2){
								$payment_type = "Bank Loan";
							} else {
								$payment_type = "Crdit";
							}
							...
							<tr>
								...
								<td><?php echo $mac_name;?></td>
								<td><?php echo $machine_type;?></td>
								...
								<td><?php echo $fetch['advance_percentage'];?></td>
								<td><?php echo $fetch['estimation'];?></td>
								<td><?php echo $fetch['projected'];?></td>
								<td><?php echo $fetch['balance'];?></td>
								<td>
								<?php 
								if($fetch['priority_set']==0)
								{
								?>
								<button type="button"  class="btn btn-info" onclick="window.location.href='set_priority.php?id=<?php echo $fetch['id']; ?>'"><a style=" text-decoration: none; color:#FFF" >Set Priority&nbsp;</a></button>
								<button type="button"  class="btn btn-info" onclick="window.location.href='remark_state.php?id=<?php echo $fetch['id']; ?>'"><a style=" text-decoration: none; color:#FFF" >Delete&nbsp;</a></button>
								<?php 
								}
								else {
								?>
								<span style="color:green"><b> Already Set </b> </span>
								<?php } ?>
								</td>
					</td>
							</tr>
