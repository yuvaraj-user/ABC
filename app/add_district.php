...
if(isset($_REQUEST['submit']))
{
	...
	$createdby=$_SESSION['usersessionid'];
		$query_aadh="SELECT COUNT(Id) as cid FROM tbl_district WHERE Name ='$name' AND Status='Active'";
$query_aadh_res = mysqli_query($con, $query_aadh); 
$fetch_cnt = mysqli_fetch_array($query_aadh_res);
 
 if($fetch_cnt['cid'] ==0) {
		
		$insert_details = mysqli_query($con,"INSERT INTO `tbl_district`(`Name`, `Parent`, `Status`, `Created_On`, `Created_By`) VALUES ('$name','$parent','$status','$createdon','$createdby')");
		
		if($insert_details){
			 echo '<script type="text/javascript">
					window.location.replace("view_district.php?step=suces");
					</script>';	
		 ...
		 echo '<script type="text/javascript">
					window.location.replace("view_district.php?step=fail");
					</script>';
	}
} else {
     echo '<script type="text/javascript">
					window.location.replace("view_district.php?step=duplicate");
					</script>';
}
}
?>
...
<div class="panel-heading lead ">
	<div class="row">
		<div class="col-lg-8 col-md-8"><b>Add District</b></div>				
			<div class="col-lg-4 col-md-4 text-right">
				<div class="btn-group text-center">
					<a href="view_district.php" class="btn btn-success"><i class="fa fa-eye"></i>&nbsp;&nbsp;&nbsp;View District</a>
				</div>
			</div>
	</div>
</div>
                ...
                <div class="panel-body"> 
                  <form class="form-horizontal" name="addaccount_details" action="" method="post" enctype="multipart/form-data" >
	                ...
	                <label for="inputName" class="col-sm-2 control-label">Parent<font color="red">&nbsp;*&nbsp;</font></label>
						 ...
						 <?php 
						    $select_GrpQry=mysqli_query($con,"select Name from tbl_district WHERE Status='Active'");
