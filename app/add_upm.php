...
if(isset($_REQUEST['submit']))
{
	...
	$createdby=$_SESSION['usersessionid'];
			...
			$query_aadh="SELECT COUNT(Id) as cid FROM tbl_uom WHERE Name ='$name' AND Status='Active'";
$query_aadh_res = mysqli_query($con, $query_aadh); 
$fetch_cnt = mysqli_fetch_array($query_aadh_res);
 
 if($fetch_cnt['cid'] ==0) {
		
		$insert_details = mysqli_query($con,"INSERT INTO `tbl_uom`(`Name`, `Formal_Name`, `Decimal_UOM`, `Created_On`, `Created_By`, `Status`) VALUES ('$name','$formal_name','$decimal','$createdon','$createdby','$status')");
       ...
       echo '<script type="text/javascript">
					window.location.replace("view_upm.php?step=duplicate");
					</script>';
 }
}
?>
                ...
                <div class="panel-body"> 
                    ...
                    <form class="form-horizontal" name="addaccount_details" action="" method="post" enctype="multipart/form-data" >
					  ...
					  <div class="form-group col-sm-12">
						 ...
						 <label for="inputName" class="col-sm-2 control-label">Decimal</label>
						 <div class="col-sm-4">
							<input type="text" placeholder="Decimal" onKeyUp="$(this).val($(this).val().replace(/[a-z`~!@#$%^&*()_|+\=?;:,'<>\{\}\[\]\\\/]/gi, ''))"  maxlength="11" id="state_code" class="form-control limited" maxlength="15" name="decimal">
						</div>
						</div>
