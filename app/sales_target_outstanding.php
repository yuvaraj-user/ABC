	...
	<div class="panel-body"> 
	 <form class="form-horizontal" name="addaccount_details" action="" method="post" enctype="multipart/form-data" >
				...
				?>
					...
					<?php } else {  ?> 
					<input type="hidden" name='product' value="<?php echo $e_id; ?>">
...
<?php
								...
								if(isset($_REQUEST['filter']))
								{
								 ...
								 $to = date("d-m-Y", strtotime($dto));							 
											...
											{
									             $query_getall2="SELECT p.*,s.Name as sup_name,t.Name as pro_name FROM tbl_sales_target p LEFT JOIN tbl_customer s on s.Id=p.Customer_Id LEFT JOIN tbl_product t on t.Id=p.Product_Id WHERE p.Status='Active' AND STR_TO_DATE(p.`Start_Date`,'%d-%m-%Y') BETWEEN STR_TO_DATE('$from', '%d-%m-%Y') AND STR_TO_DATE('$to', '%d-%m-%Y')";
											}
											else
											{
												  $query_getall2="SELECT p.*,s.Name as sup_name,t.Name as pro_name FROM tbl_sales_target p LEFT JOIN tbl_customer s on s.Id=p.Customer_Id LEFT JOIN tbl_product t on t.Id=p.Product_Id WHERE p.Status='Active' AND p.Customer_Id='$product' AND STR_TO_DATE(p.`Start_Date`,'%d-%m-%Y') BETWEEN STR_TO_DATE('$from', '%d-%m-%Y') AND STR_TO_DATE('$to', '%d-%m-%Y')";
											} 											
						...
						while($fetch=mysqli_fetch_array($qry_receipt))
							{
								...
								$qury_opn="select SUM(Quantity) as qty from tbl_sales_target where Product_Id='$product_id' AND Status='Active' AND Id='$order_id'";
								$qury_exe_opn=mysqli_query($con,$qury_opn);
								$fetch_opn=mysqli_fetch_array($qury_exe_opn);
								$opening_qty = $fetch_opn['qty'];
								
								$qury_opn_sale="select SUM(Quantity) as qty from tbl_sales where Product_Id='$product_id' AND Status='Active' AND Target_No='$order_no'";
								...
								$qury_ind="select SUM(Quantity) as qty from tbl_sales_order where Product_Id='$product_id' AND Status='Active' AND Target_No='$order_no'";
							...
							//	}else {
								...
								$qury_can="select SUM(Quantity) as qty from tbl_cancel_sales_order where Product_Id='$product_id' AND Status='Active' AND Order_Id='$order_no'";
								...
								$available_qty = ($opening_qty - $opening_qty_sale) + $overall_can_1;
			...
			"scrollCollapse": true,
			  "lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
			    ...
			    "buttons": [
			   {
				...
				{
					extend: 'excelHtml5',
					title: 'Sales Order Outstanding Product Report' 			
				}, 
				{
					extend: 'colvis',
