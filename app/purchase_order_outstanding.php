...
if(isset($_REQUEST['submit']))
{
	...
	$createdby=$_SESSION['usersessionid'];
		...
		$insert_details = mysqli_query($con,"INSERT INTO `tbl_cancel_purchase_order`(`Order_Id`, `Product_Id`, `Quantity`, `Created_On`, `Created_By`, `Status`) VALUES ('$name','$formal_name','$decimal','$createdon','$createdby','$status')");
...
<div class="row panel panel-primary"> 
	<div class="panel-heading lead ">
		<div class="row">
			<div class="col-lg-8 col-md-8">Purchase Order Outstanding Report</div>				
...
<?php
								...
								if(isset($_REQUEST['filter']))
								{
								 ...
								 $product=$_REQUEST['product'];
											...
											{
									             $query_getall2="SELECT p.*,s.Name as sup_name,t.Name as pro_name FROM tbl_purchase_order p LEFT JOIN tbl_supplier s on s.Id=p.Supplier_Name LEFT JOIN tbl_product t on t.Id=p.Product_Id WHERE p.Status='Active'";
											}
											else
											{
												 $query_getall2="SELECT p.*,s.Name as sup_name,t.Name as pro_name FROM tbl_purchase_order p LEFT JOIN tbl_supplier s on s.Id=p.Supplier_Name LEFT JOIN tbl_product t on t.Id=p.Product_Id WHERE p.Status='Active' AND p.Supplier_Name='$product'";
											} 											
...
<br>
						...
						while($fetch=mysqli_fetch_array($qry_receipt))
							{
								...
								$qury_opn="select SUM(Quantity) as qty from tbl_purchase_order where Product_Id='$product_id' AND Status='Active' AND Id='$order_id'";
								$qury_exe_opn=mysqli_query($con,$qury_opn);
								$fetch_opn=mysqli_fetch_array($qury_exe_opn);
								$opening_qty = $fetch_opn['qty'];
								
								
								 $qury="select SUM(Quantity) as qty from tbl_purchase where Product_Id='$product_id' AND Status='Active' AND Purchase_Order_No='$order_no'";
								...
								$qury_can="select SUM(Quantity) as qty from tbl_cancel_purchase_order where Product_Id='$product_id' AND Status='Active' AND Order_Id='$order_id'";
								...
								$available_qty = $opening_qty - $overall_qty_1 - $overall_can_1;
							...
							<tr>
								...
								<td><?php echo $overall_qty_1;?></td>
								<td><?php echo $overall_can_1;?></td>
								<td><?php echo $available_qty;?></td>
								<td>
			
				<ul class="para">
			 <li>
				<button type="submit" class="btn btn-success" data-toggle="modal" name="feedback" data-target="#myModal1<?php echo $order_id; ?>" data-backdrop="static" data-keyboard="false">View</button> 
				<div class="modal fade" id="myModal1<?php echo $order_id; ?>" role="dialog">
			<div class="modal-dialog">

				<!-- Modal content-->
				<div class="modal-content"  style="width:70%;height:70%;margin-left:140px">
			...
			<label for="inputName" class="col-sm-2 control-label">Date</label>
							<div class="col-sm-4">
							<input type="text" name="cancel_qty" class="form-control" value="<?php echo date("d-m-Y"); ?>" readonly>
							<input type="hidden" name="order_id" class="form-control" value="<?php echo $order_id; ?>" readonly>
							<input type="hidden" name="product_id" class="form-control" value="<?php echo $product_id; ?>" readonly>
						  </div>
						
						<label for="inputName" class="col-sm-2 control-label">Order No</label>
							<div class="col-sm-4">
							<input type="text" name="cancel_qty" class="form-control" value="<?php echo $order_no; ?>" readonly>
						  </div>
						  <label for="inputName" class="col-sm-2 control-label">Outstanding Quantity</label>
							<div class="col-sm-4">
							<input type="text" name="cancel_qty" class="form-control" value="<?php echo $available_qty; ?>" readonly>
						  </div>
						</div>
						
							<label for="inputName" class="col-sm-2 control-label">Cancel Quantity</label>
							<div class="col-sm-4">
							<input type="text" name="cancel_qty" class="form-control">
						  </div>
