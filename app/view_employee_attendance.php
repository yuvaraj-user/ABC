...
<?php
								...
								if(isset($_REQUEST['filter']))
								{
								 ...
								 $to = date("d-m-Y", strtotime($dto));	
												 ...
												 $query_getall2 ="select * from tbl_attendance where Status='Active' AND Date='$date'";
...
while ($fetch = mysqli_fetch_array($qury_exe)) {
    ...
    $qry_emp = mysqli_query($con, "SELECT Name FROM `tbl_employee` WHERE Status='Active' AND Id='$emp_id'");
    $fetch_emp = mysqli_fetch_array($qry_emp);
    $emp_name = $fetch_emp['Name'];

    $qry_brn = mysqli_query($con, "SELECT Name FROM `tbl_branch` WHERE Status='Active' AND Id='$brnch_id'");
    $fetch_brn = mysqli_fetch_array($qry_brn);
    $bname = $fetch_brn['Name'];
    ?>
                                                    <tr>
