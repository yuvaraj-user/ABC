...
if (!empty($_REQUEST["lead_id"])) {
	...
	$query_lead = "SELECT l.*,c.*,t.*,l.Date as ld_dt,s.*,s.Date as Sale_Date,l.Id as lead,l.Customer_Id as cust_id,s.Sale_Order_No as sale_order FROM tbl_sales_order s left join tbl_lead l on l.Id=s.Lead_Id left join tbl_customer c on l.Customer_Id=c.Id left join tbl_contact_person t on t.Customer_Id=c.Id WHERE s.Id='$lead_id'"; 
			...
			$sale_order = $fetch_lead['sale_order'];
...
<label for="inputName" class="col-sm-2 control-label">Sale Order Date</label>
                        ...
                        <div class="col-sm-4"> 
                            ...
                            <input type="hidden" value="<?php echo $lead_id_1; ?>" id="form-field-1" class="form-control limited" name="lead_id" readonly>
							 ...
							 <input type="hidden" value="<?php echo $sale_order; ?>" id="form-field-1" class="form-control limited" name="sale_order" readonly>
                        </div>
