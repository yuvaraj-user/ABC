...
<?php
                ...
                ?>
                    <div class="alert alert_msg alert-success alert-dismissable">
                        ...
                        <strong>Success!</strong> sale order was Added Successfully.
                    </div>
                ...
                ?>
                    <div class="alert alert_msg alert-success alert-dismissable">
                        <a href="#" class="close" data-dismiss="alert" aria-label="close">×</a>
                        <strong>Success!</strong> sale order was Updated Successfully.
                    </div>
                ...
                <?php } else if ($final == "delete") { ?>
                    <div class="alert alert_msg alert-danger alert-dismissable">
                        <a href="#" class="close" data-dismiss="alert" aria-label="close">×</a>
                        <strong>Success!</strong> sale order are Removed Successfully.
                    </div>
	...
	<div class="panel-heading lead ">
		<div class="row">
			<div class="col-lg-8 col-md-8">Sales Order Report</div>				
...
<?php
								...
								if(isset($_REQUEST['filter']))
								{
								 ...
								 $to = date("d-m-Y", strtotime($dto));							 
												 ...
												 $query_getall2.="SELECT so.*,so.Date as so_dt,l.*,l.Id as ld_id,e.Name as emp_name,t.Name as cus_name,l.Date as ld_dt,p.*,i.*,c.*,t.* FROM tbl_sales_order so left join  tbl_lead l on so.Lead_Id=l.Id LEFT JOIN tbl_lead_products p on l.Id=p.Lead_Id LEFT JOIN tbl_stock_item i on i.Id=p.Product_Id LEFT JOIN tbl_customer c on c.Id=l.Customer_Id LEFT JOIN tbl_contact_person t on t.Customer_Id=c.Id LEFT JOIN tbl_employee e on l.Followup_Employee=e.Id WHERE so.Status='Active' AND ";
												$counter=0;
																						
											if(!empty($customer))
											{
											
												$qarr[$counter]="so.Customer_Id	='$customer'";
												$counter++;
											}
											...
											if(!empty($date))
											{
												  $qarr[$counter]="STR_TO_DATE(so.`Date`,'%d-%m-%Y') BETWEEN STR_TO_DATE('$from', '%d-%m-%Y') AND STR_TO_DATE('$to', '%d-%m-%Y') GROUP BY p.Lead_Id";
    ...
    $('#example').DataTable( {
			...
			"scrollCollapse": true,
			  "lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
			   ...
			   {
					extend: 'copyHtml5',
					title: 'Sales Order Report - <?php echo $bname;?>' 			
				},
				{
					extend: 'pdfHtml5',
					title: 'Sales Order Report - <?php echo $bname;?>' 			
